...
These are the top 20 terms found. They are the ones we will use. Note this only uses ``r round(num_simp / num_genres, 3) * 100`%` of the data since ``r round(1 - num_simp / num_genres, 3) * 100`%` of the data do not fall under these top 20 categories. We concluded that for our purposes, this means that the top 20 are sufficient simplified genres to do analysis on. We chose the top 20 since after the top 20 there seemed to to be diminishing returns in adding more genres. For example, the 20th top genre ``r sm_a[1] %>% pull()``, had a count of ``r sm_a[2] %>% pull()`` which is not much more than the top 25th and 50th genres, ``r sm_b[1] %>% pull()`` and ``r sm_c[1] %>% pull()``, which have counts of ``r sm_b[2] %>% pull()`` and ``r sm_c[2] %>% pull()`` respectively. The genre with the largest count was ``r lg_a[1] %>% pull()`` with a count of ``r lg_a[2] %>% pull()``. With this in mind, we decided there wasn't much value in including these smaller genres and considered only the top 20 genres.
...
A negative correlation with r-value ``r cor(year$acousticness, year$energy)``.
...
A positive correlation with r-value ``r cor(year$loudness, year$energy)``.
...
A positive correlation with r-value ``r cor(year$tempo, year$energy)``.
...
### More Analysis {.tabset .tabset-pills}

As mentioned before, we chose the above threshold of `0.9` somewhat arbitrarily. 
This was partially motivated by not wanting to make section 3.2.3 too complicated since the number of comparisons between features grows quadratically. 
...
A negative relationship with r-value ``r cor(year$acousticness, year$loudness)``.

```{r}
year %>% 
  ggplot(aes(x = acousticness, y = loudness)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Relationship Between Acousticness and Loudness") +
  xlab("Acousticness") +
  ylab("Loudness")
```
...
A negative relationship with r-value ``r cor(year$acousticness, year$tempo)``.

```{r}
year %>% 
  ggplot(aes(x = acousticness, y = tempo)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Relationship Between Acousticness and Tempo") +
  xlab("Acousticness") +
  ylab("Tempo")
```
...
A negative relationship with r-value ``r cor(year$acousticness, year$popularity)``.
...
year %>% 
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Relationship Between Acousticness and Popularity") +
  xlab("Acousticness") +
  ylab("Popularity")
```
...
A positive relationship with r-value ``r cor(year$popularity, year$energy)``.
...
year %>% 
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Relationship Between Popularity and Energy") +
  xlab("Popularity") +
  ylab("Energy")
```
...
A negative relationship with r-value ``r cor(year$popularity, year$instrumentalness)``.
...
year %>% 
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Relationship Between Popularity and Instrumentalness") +
  xlab("Popularity") +
...
A positive relationship with r-value ``r cor(year$popularity, year$loudness)``.
...
year %>% 
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Relationship Between Popularity and Loudness") +
  xlab("Popularity") +
  ylab("Loudness")
```
...
A positive relationship with r-value ``r cor(year$loudness, year$tempo)``.
...
year %>% 
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Relationship Between Loudness and Tempo") +
  xlab("Loudness") +
  ylab("Tempo")
```


#### "Tangental Correlations"
...
```{r}
pairs(year[,c(2, 5, 6, 8, 10, 12)])
```

Table of the r-values.

```{r}
correlate(year[,c(2, 5, 6, 8, 10, 12)])
```
...
### Assumptions {.tabset .tabset-pills}
...
# feature_names1 <- colnames(condensed %>% select(-c(genres, simple_genre, key, mode)))
feature_names2 <- colnames(condensed %>% select(-c(genres, simple_genre, key, mode)))
# for (i in seq(length(feature_names1))) {
#   feature_names1[i] <- feature_names1[i] %>% substr(1, 2) %>% str_c("m_", .)
# }
# i <- 1
...
#   i <- i + 1
# }
plot_normal = function (data, name) {
  sm_name = str_c("m_",  substr(name, 1, 2))
  data %>%
    ggplot(aes_string(x = sm_name)) +
    geom_density() +
    ggtitle(
      label = str_c("Distribution of ", name, " means")
    ) +
    xlab(str_c("mean ",name))
}
plots <- list()
plots <- lapply(X = c(feature_names2), FUN = plot_normal, data = foo)
```
...
We ran t-tests to find differences between genres in the different features we considered normal. The t-test statistic^[[t-test formula](http://www.sthda.com/english/wiki/t-test-formula)] is as follows:
...
condensed %>%
  ggplot(aes(
    y = reorder(simple_genre, popularity),
    x = popularity,
    fill = simple_genre
  )) +
  geom_boxplot() +
...
<!-- We found the most popular by finding the genre with the highest mean popularity. Here you can see the genre with the highest mean popularity (m_po) was `rap` (sd_po is standard deviation of popularity). -->
...
#### t-test p-value tables {.tabset .tabset-pills}

##### All t-tests for popularity.
...
Next we isolated the genres that didn't have `p-value < 0.05` to find the genres cannot be dismissed as having a different popularity from rap.
...
<!-- The following boxplot shows the popularity for hip, rap, and swedish. -->
...
condensed %>%
  ...
  geom_boxplot() +
  geom_hline(yintercept = 21-3+.5, color="red", linetype="dashed") +
  ylab("Simple Genre") +
  ggtitle(
    label = "Most Popular Genres"
    )
...
```{r more tests to distinguish}
# This is additional analysis to not just distinguish the MOST popular genres, but instead to divide the genres into groups of popularity. This was done manually and was somewhat tedious so we did not include it in the actual report.
...
arr["indie"] %>% 
  filter(indie > 0.05) # lowest pop is japanese

arr["metal"] %>% 
  filter(metal > 0.05) # lowest pop is folk

arr["classical"] %>% 
  filter(classical > 0.05) # lowest pop is italian
...
condensed %>%
  ...
  geom_boxplot() +
  geom_hline(yintercept = 21-3+.5, color="red", linetype="dashed") +
  geom_hline(yintercept = 21-8+.5, color="red", linetype="dashed") +
  geom_hline(yintercept = 21-13+.5, color="red", linetype="dashed") +
  geom_hline(yintercept = 21-19+.5, color="red", linetype="dashed") +
  geom_hline(yintercept = 21-20+.5, color="red", linetype="dashed") +
  ylab("Simple Genre") +
  ggtitle(
    label = "Most Popular Genres"
    )
...
feature_names2 <- colnames(condensed %>% select(-c(genres, simple_genre, key, mode)))
for (i in seq(length(feature_names))) {
  feature_names[i] <- feature_names[i] %>% substr(1, 2) %>% str_c("m_", .)
}
i <- 1
for (f in feature_names) {
  gr <- foo %>%
